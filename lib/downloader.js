...
class Downloader {
  ...
  async saveAllFiles() {
    let saved = 0;
    const total = this.pages.size + this.assets.size;

    // Save HTML pages
    for (const [url, pageData] of this.pages.entries()) {
      try {
        this.ensureDirectoryExists(pageData.localPath);
        fs.writeFileSync(pageData.localPath, pageData.html, 'utf-8');
        saved++;

        this.logger.progress(`Saving: ${saved}/${total} files`);
      } catch (error) {
        this.logger.error(`Failed to save ${url}: ${error.message}`);
      }
    }

    // Save assets
    for (const [url, assetData] of this.assets.entries()) {
      try {
        this.ensureDirectoryExists(assetData.localPath);
        fs.writeFileSync(assetData.localPath, assetData.content);
        saved++;
    ...
    this.logger.clearProgress();
    this.logger.success(`Saved ${saved} files to disk`);
  }
