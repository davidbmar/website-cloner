...
class LinkRewriter {
      ...
      try {
        ...
        $('a[href]').each((i, el) => {
          ...
          if (newHref !== href) {
            ...
            // Add title attribute for 404 redirects
            if (newHref === './404.html') {
              $(el).attr('title', `Page not included in clone (depth: ${this.config.crawling.maxDepth})`);
            }
        ...
        // Save rewritten HTML back to disk
  ...
  rewriteUrl(url, baseUrl, currentFilePath, urlMap) {
    ...
    if (!targetFilePath) {
      // URL wasn't downloaded - redirect to 404 page
      this.stats.redirectedTo404++;
      return './404.html';
    }
  ...
  displaySummary() {
    ...
    console.log(`Links redirected to 404:     ${this.stats.redirectedTo404}`);
