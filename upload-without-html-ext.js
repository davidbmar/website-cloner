...
async function uploadFiles() {
  console.log(`Step 2: Uploading files to '${s3Prefix}/' without .html extension...\n`);
  ...
  for (const filePath of files) {
    ...
    let s3Key = relativePath.replace(/\\/g, '/');

    // Remove .html extension from S3 key (but keep index.html and 404.html)
    if (s3Key.endsWith('.html') && s3Key !== 'index.html' && s3Key !== '404.html') {
      s3Key = s3Key.slice(0, -5); // Remove last 5 characters (.html)
    }

    // Add prefix for subdirectory
