...
class S3Uploader {
  ...
  async uploadFile(filePath, localDir) {
    ...
    // Skip portfolio index.html when uploading to a site prefix
    // The portfolio page belongs at the bucket root, not in individual site directories
    if (this.config.s3.prefix && relativePath === 'index.html') {
      this.logger.debug('Skipping portfolio index.html (belongs at bucket root, not in site prefix)');
      return; // Skip this file
    }

    // Skip manifest files when uploading to a site prefix
    // These are metadata files that shouldn't be in the deployed site
    if (this.config.s3.prefix && (relativePath === 'manifest.json' || relativePath === 'dynamic-manifest.json')) {
      this.logger.debug(`Skipping ${relativePath} (metadata file)`);
      return; // Skip this file
    }
